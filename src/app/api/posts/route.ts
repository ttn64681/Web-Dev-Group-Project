...
export async function POST(request: NextRequest) {
  ...
  try {
    const { title, description, url, thumbnail, postType, courseId, userId, username } =
      await request.json();
    console.log(
      `INFO DUMP: ${title}, ${description}, ${url}, ${thumbnail}, ${postType}, ${courseId}, ${userId}`
    );
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Missing required fields: title, description, url, postType, and course are required',
        },
        { status: 400 }
      );
    }
